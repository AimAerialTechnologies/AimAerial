...
@section('breadcrumb')
  ...
  <li class="breadcrumb-item active">Drones List</li>
@endsection
          ...
          <div class="table-responsive">
            <table id="basic-datatable" class="table nowrap w-100">
              <thead>
                <tr>
                  @php
                  $titleArr = ['#' , 'Drone ID', 'Image', 'Drone Model' , 'Serial Number' , 'IMEI', 'Description', 'Owner', 'Status', 'Drone URL', 'QR Code', 'Action'];
                  @endphp
                ...
                @isset ($drones)
                  @foreach ($drones as $key => $drone)
                    <tr>
                      ...
                      <td><img src="{{ $drone->parse_image}}" alt="{{ $drone->parse_image}}" width="50" height="50" onclick="previewImg(this)" data-bs-toggle="modal" data-bs-target="#viewImg" /> <p class="d-none">{{ $drone->parse_image}}</p> </td>
                      <p id="image-{{$n}}" class="d-none">{{$drone->parse_image}}</p>
                      ...
                      <td>
                        @if ($drone->user)
                          <a id="owner-{{$n}}" href="{{ route('users.show', ['user' => $drone->user->hashed_id]) }}">{{$drone->user->name}}</a>
                          <p id="ownerHash-{{$n}}" hidden>{{$drone->user->hashed_id}}</p>
                        @endif
                      </td>
                      ...
                      <td>
                        <div class="form-check form-switch">
                          <input type="checkbox" class="form-check-input" id="status-{{$n}}" onclick="statCheck(this)" data-id="{{ $n }}" {{$drone->status == "Enable"?"checked":""}} >
                          <label class="form-check-label {{$drone->status == "Disable"?"text-danger":"text-success"}}" id="status-name-{{$n}}" for="status-{{$n}}">{{$drone->status}}</label>
                        </div>
                      </td>
                      <td> <a href="{{route('drones.get',['id' => Helper::hash($drone->id,50)] )}}" target="_blank">Drone URL</a>
                      </td>
                      <td> <a href="{{route('drones.qr',['id' => Helper::hash($drone->id,50)] )}}" target="_blank">QR Code</a> </td>
                      <td>
        ...
        <div class="modal-body">
          <form>
            <div class="form-group row">
              ...
              <div class="col-sm-6">
                <img src="" id="detail-image" class="col-form-label" width="50" height="50" onclick="previewImg(this)" data-bs-toggle="modal" data-bs-target="#viewImg" />
              </div>
            </div>
            <div class="form-group row">
          ...
          <form method="POST" action="{{route('drones.store')}}" enctype="multipart/form-data" >
            ...
            <div class="mb-3 col-12">
              <label for="owner">Owner
                @role('Admin')
                <small><a href="{{ route('users.index') }}">Add Owner</a></small>
                @endrole
              </label>
              <select name="user" class="form-control" id="user">
                <option value="">Select Owner</option>
                @foreach ($users as $key => $user)
                  <option value="{{ $user->hashed_id }}">{{ $user->name }}</option>
                @endforeach
              </select>
            </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="POST" action="{{route('drones.edit')}}" enctype="multipart/form-data">
          ...
          <div class="mb-3 col-12">
            <label for="edit-image" class="form-label">Upload Image Here</label>
            <input class="form-control" type="file" name="image" id="edit-image" accept="image/jpeg,image/png">
            <small>Size limit 5mb. Image format must be in .png or .jpeg</small>
          </div>
          <div class="mb-3 col-12">
          ...
          <div class="mb-3 col-12">
            <label for="edit-serial_num">Serial Number</label>
            <input type="text" class="form-control" id="edit-serial_num" name="serial_num">
          </div>
          <div class="mb-3 col-12">
            <label for="edit-imei">IMEI</label>
          ...
          <div class="mb-3 col-12">
            ...
            <select id="edit-owner" name="user" class="form-control">
              @foreach ($users as $key => $user)
                <option value="{{ $user->hashed_id }}">{{ $user->name }}</option>
              @endforeach
            </select>
...
{{-- View Img Modal begin --}}
<div class="modal fade" id="viewImg" tabindex="-1" role="dialog" aria-hidden="true">
...
@section('add-script')
  ...
  function statCheck(d){
    var keyID = d.getAttribute("data-id");
    if(d.checked){
      $("#status-name-"+keyID).text("Enable");
      $("#status-name-"+keyID).removeClass( "text-danger" );
      $("#status-name-"+keyID).addClass( "text-success" );
    }
    else {
      $("#status-name-"+keyID).text("Disable");
      $("#status-name-"+keyID).removeClass( "text-success");
      $("#status-name-"+keyID).addClass( "text-danger");
    ...
    var status = $('#status-name-'+keyID).text();
    ...
    var status = $('#status-name-'+keyID).text();
    var owner = $('#owner-'+keyID).text();
    var ownerHash = $('#ownerHash-'+keyID).text();
    $('#edit_id').val(dId);
    $('#edit-model').val(model);
    $('#edit-serial_num').val(serial_num);
    $('#edit-imei').val(imei);
    $('#edit-description').val(description);
    $('#edit-owner').val(ownerHash);
    if (status=="Disable") {
