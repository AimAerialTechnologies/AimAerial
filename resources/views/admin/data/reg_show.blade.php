...
@section('content')
  <div class="row">
    <div class="col-6">
      <div class="card">
        <div class="card-body">
          <h4 class="header-title">{{$user->name}}'s Access List</h4>
          <p class="sub-header">Assign role access list below</p>
          <form method="POST" action="#">
            @csrf
            <input name="role" value="{{$user->HashedId}}" hidden>
            <div class="row">
              <div class="mb-3 col">
            @if ($columns)
              <div class="form-check">
                <label for="unCheck" class="form-label">Uncheck All</label>
                <input id="unCheck" class="form-check-input" type="checkbox" value="{{Helper::hash(0)}}" name="column[]" onclick="unCheckItem(this)" {{--$user->columns->count()?'':'checked'--}}>
              </div>
            @endif
            @foreach ($columns as $key => $column)
              <div class="form-check">
                <label for="{{$column}}" class="form-label">{{$column}}</label>
                <input class="form-check-input" type="checkbox" value="{{$column}}" id="{{$column.$key}}" name="column[]" {{-- $user->hascolumnTo($column->name) ? 'checked':''--}} data-item-name="column" onclick="checkItem(this)">
              </div>
            @endforeach
            </div>
            <div class="row">
